...
int main_handler(int type, int par1, int par2) {
	if (type == EVT_INIT) {
		...
		DrawBoard();
		/*
		sprintf(message, "baseX is %d, baseY is %d", baseX, baseY);
		Message(ICON_INFORMATION, "Message", message, 10000);
		*/
		...
		sprintf(message, "Event type %d", type);
		Message(ICON_INFORMATION, "Message", message, 5000);
		*/
	}
	return 0;
}
